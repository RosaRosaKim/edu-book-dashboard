...
### 가게 선택지
- **당산 바나프레소** — 메뉴 UI 제공 (바나프레소 API 직접 연동)
- **수동입력** — 차후 구현 예정

### 기본 플로우
```
[생성자] 가게 선택 (당산 바나프레소)
  → 로딩 (사용자 목록 프리로드 + 메뉴 데이터)
...
| 액션 | 핸들러 | 설명 |
|------|--------|------|
| `createItsOnMe` | `handleCreateItsOnMe` | 세션 생성 + Flow 발송 |
...
| 메시지 | 함수 | 내용 |
|--------|------|------|
| 초대 | `itsOnMeInvite` | `{이름} 프로가 메뉴를 골라달래` / `{가게} {시간}까지 메뉴선택해` |
| 세션 변경 | `itsOnMeUpdate` | `{이름} 프로가 {액션}` (멤버 추가, 시간 연장 시 발송자에게) |
| 마감 임박 | `itsOnMeReminder` | `아직 메뉴를 안 골랐어! {시간}에 마감이야.` / `{가게} 마감 5분 전!` |
| 취합 결과 | `itsOnMeSummary` | 메뉴별 그룹핑 + 인원 + 총금액 |
...
| 파일 | 역할 |
|------|------|
| `src/gas/its-on-me.gs` | 세션 CRUD, 멤버 추가, 시간 연장, 폴링 |
| `src/gas/banapresso-sync.gs` | 바나프레소 API 연동, 메뉴 캐싱 |
| `src/gas/flow-messages.gs` | 이쏜미 관련 Flow 메시지 정의 |
| `src/html/emro-life.html` | 프론트엔드 전체 (가게선택, 멤버선택, 메뉴선택, 옵션) |
...
`its-on-me.gs`에 3곳 주석처리됨 (본인 제외 로직):
1. **멤버 검색 본인 제외** (line ~36): `// if (knoxId.toLowerCase() === myKnox) continue;`
2. **이전 멤버 본인 제외** (line ~99): `// if (rKnox.toLowerCase() === myKnox) continue;`
3. **Flow 발송 생성자 제외** (line ~167): `// if (allMembers[k].knoxId.toLowerCase() === creatorKnox.toLowerCase()) continue;`
...
| 이쏜미응답 시트 F열 추가 | 기존 데이터 있으면 | 가격 컬럼 삽입, 선택시각 G열로 이동 |
